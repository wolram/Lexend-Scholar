...
- LS-142 Definir políticas financeiras do produto
- LS-143 Configurar email marketing e sequência de onboarding (7 emails)
- LS-187 Definir palavras-chave SEO e criar calendário editorial
- LS-192 remover Tailwind CDN, referenciar CSS compilado
- LS-197 Setup Next.js 14 App Router na webapp
- LS-193 endpoint POST /api/contato com Supabase + Resend
- LS-198 Login page + middleware de autenticação por role
- LS-205 credenciais Supabase via xcconfig, remover hardcode
- LS-199 Dashboard com métricas reais
- LS-194 formulário de contato funcional via fetch /api/contato
- LS-200 Módulo Alunos — lista + perfil
- LS-206 FinancialService.swift conectado ao Supabase com fallback DemoData
- LS-201 Módulo Turmas + lançamento de frequência
- LS-195 about, blog e careers com conteúdo real
- LS-196 vercel.json adiciona rewrite /api/* para webapp
- LS-207 views financeiras conectadas ao FinancialService
- LS-202 módulo financeiro web (cobranças, inadimplência, relatório)
- LS-203 portal do responsável (boletim, frequência, cobranças)
- LS-204 deploy webapp Vercel — vercel.json + env.example
- LS-209 fluxo login completo → AppShell
...
- Enterprise README with tech badges
...
- Atualizar CHANGELOG.md [skip ci]
- Atualizar CHANGELOG.md [skip ci]
- LS-208 xcodegen generate — projeto regenerado
- Atualizar CHANGELOG.md [skip ci]
- Atualizar CHANGELOG.md [skip ci]
- Adicionar Info.plist e entitlements gerados pelo xcodegen
- Atualizar CHANGELOG.md [skip ci]
- Atualizar CHANGELOG.md [skip ci]
- *(ci)* Add branch-based release automation
## [unreleased]
